...
function printRotationUsage(logInfo: (message: string) => void): void {
	logInfo(
		[
			...
			"",
			"Behavior:",
			"  - Wrapper-launched Codex sessions use native selected-account auth homes by default",
			"  - Runtime rotation is opt-in and routes request-bearing Codex sessions through a localhost proxy",
			"  - Binds the packaged Codex desktop app to the same localhost router when enabled or repaired",
			"  - Use CODEX_MULTI_AUTH_RUNTIME_ROTATION_PROXY=1 to enable the proxy for the current process without changing persistent settings",
