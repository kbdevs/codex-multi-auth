...
- Runtime rotation proxy is loopback-only, opt-in, and authenticated with a local client key. Users can enable it with `codex-multi-auth rotation enable` or `CODEX_MULTI_AUTH_RUNTIME_ROTATION_PROXY=1`.
